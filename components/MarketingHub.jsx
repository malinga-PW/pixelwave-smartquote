      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 glass-panel rounded-2xl p-6 border border-slate-800/80 space-y-5">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Message Content (SLA Template)</label>
              <textarea
                rows="4"
                value={campaignMsg}
                onChange={(e) => setCampaignMsg(e.target.value)}
                className="w-full bg-slate-950/40 border border-slate-850 rounded-xl p-3 text-xs text-slate-100 placeholder-slate-600 focus:outline-none focus:border-brand-blue leading-relaxed font-mono"
              ></textarea>
              <span className="text-[9px] text-slate-500">Variables like [Client] are parsed automatically by the n8n AI node.</span>
            </div>

            {/* Recipients check-list */}
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Select Clients</label>
                ...
                {clients.map(c => {
                  ...
                  return (
                    <div 
                      ...
                      className={`flex items-center gap-2.5 px-3 py-2 rounded-xl border text-xs cursor-pointer transition-all duration-300 ${
                        isChecked 
            ...
            <div className="pt-2">
              {isSending ? (
                ...
                <button
                  ...
                  className="w-full py-3 rounded-xl bg-gradient-to-r from-brand-blue to-brand-pink hover:from-brand-blue/95 hover:to-brand-pink/95 text-white text-xs font-bold flex items-center justify-center gap-2 shadow-lg shadow-brand-blue/10 transform hover:-translate-y-0.5 transition-all"
                >
        ...
        <div className="lg:col-span-5 space-y-6">
            ...
            <div className="grid grid-cols-3 gap-2.5 pt-2 border-t border-slate-900 text-center">
              {chartData.map((item, idx) => (
                <div key={idx} className="space-y-1">
                  ...
                  <span className="text-[10px] text-slate-400 block font-medium">{item.name.split(' / ')[0]}</span>
          ...
          <div className="glass-panel rounded-2xl p-5 border border-slate-800/80 text-xs text-slate-400 leading-relaxed font-medium">
